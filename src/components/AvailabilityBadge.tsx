  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-2xl border p-4 shadow-lg ${
        available ? 'border-emerald-250 bg-emerald-50 text-emerald-950' : 'border-stone-200 bg-stone-50 text-stone-600'
      } ${className}`}
    >
      <span className="relative flex h-2.5 w-2.5">
        {available && <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />}
        <span className={`relative inline-flex h-2.5 w-2.5 rounded-full ${available ? 'bg-emerald-500' : 'bg-stone-400'}`} />
      </span>
