...
export default function ServiceDetail({
  ...
  // Sticky CTA bar: shown once the hero (and its inline CTAs) scroll out of view.
  const { ref: heroRef, isIntersecting: heroInView } = useIntersectionObserver({ threshold: 0 })
  // Mirror the navbar's hide-on-scroll so the bar rides below it when shown and
  // rises to the top when it hides (same hook → same state).
  ...
  return (
    <article className="text-left pb-8">
      {/* Sticky CTA bar — slides in once the hero is out of view */}
      <div
        className={cn(
